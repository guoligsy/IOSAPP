...
@synthesize delegate, tableView,gridView,datePickerView;
...
- (void)addSubviewsToHeaderView:(UIView *)headerView
{
  ...
  // Add column labels for each weekday (adjusting based on the current locale's first weekday)
   NSDateFormatter* ft = [[[NSDateFormatter alloc] init] autorelease];
    [ft setLocale:[[[NSLocale alloc] initWithLocaleIdentifier:@"zh_CN"] autorelease]];
  NSArray *weekdayNames = [ft shortWeekdaySymbols];//add by Even zheng
...
//  [contentView addSubview:tableView];
    ...
    datePickerView = [[UIDatePicker alloc] initWithFrame:fullWidthAutomaticLayoutFrame];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
  if (object == gridView && [keyPath isEqualToString:@"frame"]) {
    ...
    tableView.frame = frame;
      ...
      frame = datePickerView.frame;
      frame.origin.y = gridBottom;
      frame.size.height = datePickerView.superview.height - gridBottom;
