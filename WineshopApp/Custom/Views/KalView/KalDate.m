...
- (NSDate *)NSDate
{
  ...
  c.month = a.month;
  c.year = a.year;
    //add by Even zheng fix bug
    NSDateFormatter* dateFt = [[[NSDateFormatter alloc] init] autorelease];
    [dateFt setDateFormat:@"yyyy-MM-dd"];
    NSString* str = [dateFt stringFromDate:[[NSCalendar currentCalendar] dateFromComponents:c]];
    //在转换成NSDate之前，要先转换成GMT时区,否则会帮你减去8小时
    [dateFt setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:0]];
    return [dateFt dateFromString:str];
}
