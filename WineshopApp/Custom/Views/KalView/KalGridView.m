...
//@synthesize selectedTileViewDelegate = _selectedTileViewDelegate;
  ...
  if (selectedTile != tile) {
    ...
    tile.selected = YES;
      ...
      for (id subview in [[self superview] subviews]) {
          if ([subview isMemberOfClass:[UIDatePicker class]]) {
              UIDatePicker* datePicker = (UIDatePicker*)subview;
              [delegate didSelectDate:tile.date andTime:[datePicker date]];
              break;
          }
      }
      NSLog(@"selectedDate ,year = %d,month = %d,day = %d",tile.date.year,tile.date.month,tile.date.day);
  }
}
